...
class AlibabaCrawlerSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = ['https://www.alibaba.com/products/doors.html?spm=a2700.galleryofferlist.pagination.1.2669189flzhOGI&IndexArea=product_en&page='+str(i) for i in range(1, 5)]
    ...
    def parse(self, response):
        ...
        seller_years = products.xpath(XPATH_SELLER_YEARS).extract()
        ...
        result = zip(product_name, supplier_name)
        
        for product, supplier in result:
            item = ScrapyAlibabaItem()
            item['product'] = product
            item['supplier'] = supplier
            item['seller_years'] = seller_years
            yield item
